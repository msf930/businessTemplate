...
const ABOUT_QUERY = `*[_type == "aboutPage"] {_id, whoWeAre { whoWeAreText, whoWeAreTeam[]{memberTitle, memberBio, name, teamMemberImage { asset -> { _id, url } } } } }`;
                ...
                {data[0]?.whoWeAre?.whoWeAreTeam?.length && data[0]?.whoWeAre?.whoWeAreTeam?.map((item, index) => (
                        <div className="aboutBioItem" key={index}>
                            <div className="aboutBioImage">
                                <Image src={item.teamMemberImage?.asset?.url} alt={item.name} fill objectFit="cover"/>
                            </div>
                            <h2>{item.name}</h2>
                            <h3>{item.memberTitle}</h3>
                            <p>{item.memberBio}</p>
                        </div>
                        )
                    )
